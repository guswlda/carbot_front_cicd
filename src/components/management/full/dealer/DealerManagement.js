...
const DealerManagement = () => {
  ...
  const handleEditClick = (dealer) => {
    ...
    setNewPhone(dealer.dealer_phone); // 기존 핸드폰 번호를 입력란에 기본값으로 설정
    setIsEditing(false); // 전화번호 수정 비활성화 상태로 시작
  };
  ...
  const handleSavePhone = async () => {
    try {
      const response = await axios.patch(
        `http://222.112.27.120:8001/update_dealer_phone/${selectedDealer.dealer_no}`,
        {
